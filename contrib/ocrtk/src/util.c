...
static Boolean split_option(arg, next_arg, option)
	char *arg, *next_arg;Option option[]; {
	short i;
	if (!option)
		goto invalid_option;
	...
	invalid_option: error_string("invalid option", arg, Exit);
	duplicate_option: error_string("duplicate option", arg, Exit);
	return False;
}
/**********************************************************************/

static void parse_args(argc, argv, usage, option)
	int *argc;char *argv[], *usage;Option option[]; {
	short i, j = 0;
...
void error_status(message, status, exit)
	char *message;int status;Boolean exit; {
...
void error_string(message, string, exit)
	char *message, *string;Boolean exit; {
