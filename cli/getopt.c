...
static const char *_getopt_initialize(int argc, char **argv,
		const char *optstring, struct _getopt_data *d, int posixly_correct) {
...
# define NONOPTION_P (argv[d->optind][0] != '-' || argv[d->optind][1] == '\0')
	...
	if (d->__nextchar == NULL || *d->__nextchar == '\0') {
		...
		if (d->__ordering == PERMUTE) {
			...
			if (d->__first_nonopt != d->__last_nonopt && d->__last_nonopt
					!= d->optind)
		...
		d->__nextchar = (argv[d->optind] + 1 + (longopts != NULL
				&& argv[d->optind][1] == '-'));
	}
	...
	if (longopts != NULL && (argv[d->optind][1] == '-' || (long_only
...
%s: option '%s' requires an argument\n"),
		...
		if (!long_only || argv[d->optind][1] == '-' || strchr(optstring,
				*d->__nextchar) == NULL) {
			if (print_errors) {
#if defined _LIBC && defined USE_IN_LIBIO
				char *buf;
...

	{
		...
		if (temp[0] == 'W' && temp[1] == ';') {
			...
			for (p = longopts, option_index = 0; p->name; p++, option_index++)
				if (!strncmp(p->name, d->__nextchar, nameend - d->__nextchar)) {
					if ((unsigned int) (nameend - d->__nextchar) == strlen(
							p->name)) {
						/* Exact match found.  */
						pfound = p;
						indfound = option_index;
...
int getopt(int argc, char * const *argv, const char *optstring) {
	return _getopt_internal(argc, (char **) argv, optstring,
			(const struct option *) 0, (int *) 0, 0, POSIXLY_CORRECT);
}
