...
static short key_index(unsigned char *key)
{
    long i, sum = 0;
    for (i = 0; key[i]; i++)
	sum += key[i];
...
{
    Entry *entry;
    for (entry = table->list[key_index(key)].first; entry &&
...
Entry *entry;
{
    list_insert_first(&table->list[key_index(entry->key)], entry);
    free_array(table);
    table->count++;
}
/**********************************************************************/
...
Entry *entry;
{
    list_remove(&table->list[key_index(entry->key)], entry);
    free_array(table);
    table->count--;
}
/**********************************************************************/
