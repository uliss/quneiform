...
CRtfFragment * CRtfFragment::read(FILE * in) {
    assert(in);
    ...
    frag->setType(FT_TEXT);
    fread(&RectFragm, 1, sizeof(Rect16), in);
    frag->m_rect.left = RectFragm.left;
    frag->m_rect.top = RectFragm.top;
...
void CRtfFragment::FWritePicture(int NumberCurrentFragment, SectorInfo *SectorInfo, Bool OutPutType) {
