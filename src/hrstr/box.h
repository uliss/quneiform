...
enum
{
    // flag for temporary box marking
    BOXMARK = 1,
    // the picture box - only header in the box is true
    BOXPICTURE = 2,
    // line start type mask
    BOXFREEBEG = 0x30,
    // line end type mask
...
const int BOXHLTH = sizeof(BOX); // the box header length
const int BOXSIZE = BOXHLTH + 32 * 4; // the length of box
const int BOXBOUNDARY = BOXSIZE - 6; // the boundary control
